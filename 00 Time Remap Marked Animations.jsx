// This script looks at a precomp's markers and creates keyframes for each animation using time remapping
{
    ...
    function refreshKeyframes(layer, markers) {
        ...
        if (timeRemap.numKeys % 2 !== 0) {
            alert('The layer must have an even number of time-remap keyframes');
            return;
        }

        // Create keyframes for each pair of markers
        var lastEnd = timeRemap.keyTime(timeRemap.numKeys) + secondsBetweenAnims;
        var markerIndexStart = timeRemap.numKeys + 1;
        for (var i = markerIndexStart; i <= markers.numKeys; i += 2) {
    ...
    function createKeyframes(layer, markers) {
        ...
        // Remove the time 0 keyframe
        for (var i = 1; i <= timeRemap.numKeys; i++) {
            if (timeRemap.keyValue(i) === 0) {
                timeRemap.removeKey(i);
                break;
            }
        }
    ...
    function main() {
        ...
        for (var i = 0; i < activeItem.selectedLayers.length; i++) {
            ...
            if (markers.numKeys < 2 || markers.numKeys % 2 !== 0) {
                alert('The precomp must have at least two markers and an even number of markers.');
                return;
            }
            ...
            if (selectedLayer.timeRemapEnabled) {
                ...
                // Delete all markers
                while (selectedLayer.marker.numKeys >= 1) {
                    selectedLayer.marker.removeKey(1);
                }
